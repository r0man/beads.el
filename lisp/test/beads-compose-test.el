...
(ert-deftest beads-compose-test-submit-create ()
  "Verify C-c C-c in create buffer calls beads-command-execute."
  (let ((created-cmd nil))
    (cl-letf (((symbol-function 'beads-command-execute)
               (lambda (cmd)
                 (when (cl-typep cmd 'beads-command-create)
                   (setq created-cmd cmd))
      ...
      (let ((buf (beads-compose-create)))
        (unwind-protect
            (with-current-buffer buf
              ...
              (should created-cmd)
              (should (equal (oref created-cmd title)
                             "Test Issue"))
              (should (equal (oref created-cmd description)
                             "This is the description."))
              (should (equal (oref created-cmd issue-type)
                             "bug"))
              (should (equal (oref created-cmd priority) "1")))
