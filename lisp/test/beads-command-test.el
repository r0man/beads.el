...
;; These tests use beads-test-create-project to create temporary
...
Integration test that runs real bd create command."
  ...
  (let* ((default-directory (beads-test-create-project))
         (issue (beads-command-create! :title "Test issue")))
...
Integration test that verifies dependencies are set correctly."
  ...
  (let* ((default-directory (beads-test-create-project))
         ...
         (parent-issue (beads-command-create! :title "Parent issue"))
...
Integration test that filters issues by type."
  ...
  (let* ((default-directory (beads-test-create-project))
         ...
         (bug-issue (beads-command-create!
                     :title "Bug issue"
                     :issue-type "bug"))
...
Integration test that applies multiple filters together."
  ...
  (let* ((default-directory (beads-test-create-project))
         ...
         (target-issue (beads-command-create!
                        :title "Critical bug"
                        :issue-type "bug"
                        :priority "0"))
...
Integration test that filters issues by assignee."
  ...
  (let* ((default-directory (beads-test-create-project))
         ...
         (alice-issue (beads-command-create!
                       :title "Alice's task"
                       :assignee "alice"))
...
Integration test that verifies empty list is returned correctly."
  ...
  (skip-unless (executable-find beads-executable))
  (let* ((default-directory (beads-test-create-project))
         (issues (beads-command-list! :issue-type "epic")))
    (should (listp issues))
