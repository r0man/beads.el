...
;; - Full 1.3.0 flag surface: --long, --as-of, --refs, --children,
;;   --local-time, --include-dependents, --brief-deps, and
;;   --include-comments (JSON-only data flags)
;;
;; Usage:
...
to render the CHILDREN and BLOCKS sections in the show buffer.")
   (brief-deps
    ...
    :group "Options"
    :level 3
    :documentation "Reduce each dependency to its identity fields in the result.
JSON-only flag: drops description, design, notes and acceptance
criteria from dependency entries, trading detail for payload size.
bd 1.3.0 flag (bd show --help).")
   (include-comments
    :type boolean
    ...
    :group "Options"
    :level 3
    :documentation "Stream full comment bodies in the result.
JSON-only flag: plain `bd show --json' omits comment bodies, so this
flag is required to render the COMMENTS section in the show buffer.
May be slow on issues with many comments (bd 1.3.0 help note)."))
...
Transient levels control which options are visible (cycle with C-x l):
  ...
  Level 2: Thread, long
  Level 3: as-of, current, local-time, watch, include-dependents,
           brief-deps, include-comments (bd 1.3.0 JSON-only flags)"
