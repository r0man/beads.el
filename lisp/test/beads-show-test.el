...
(ert-deftest beads-show-test-insert-labels-empty ()
  "Test that absent/empty labels skip the section entirely.
An absent `labels' key (nil) and an explicitly empty list get the
same hide-when-empty treatment: no header and no placeholder."
  (with-temp-buffer
...
(ert-deftest beads-show-test-insert-dependencies-section-empty ()
  "Test that empty dependencies skip the section entirely."
  (with-temp-buffer
    (let ((inhibit-read-only t))
  ...
  (with-temp-buffer
    (let ((inhibit-read-only t)
          (deps (list (beads-dependency
                       ...
                       :type "related"))))
      (beads-show--insert-dependencies-section deps)
      ;; related type is filtered out, leaving nothing to render: the
  ...
  (beads-show-test-with-temp-buffer
   (let ((parsed (beads--parse-issue beads-show-test--minimal-issue)))
     ...
     (let ((text (beads-show-test--get-buffer-text)))
       ...
       (should (string-match-p "BUG" text))
       ;; Empty body sections are skipped entirely: no headers and no
       ;; "(none)" placeholders anywhere in the buffer
       ...
       (should-not (string-match-p "(none)" text))))))
...
(ert-deftest beads-show-test-render-empty-sections-skipped ()
  "Test that empty sections are skipped entirely (no header, no placeholder)."
  (beads-show-test-with-temp-buffer
   (let* ((issue '((id . "bd-50")
                  (title . "No details")
                  (status . "open")
     ...
     (let ((text (beads-show-test--get-buffer-text)))
       ...
       (should-not (string-match-p "Design" text))
       (should-not (string-match-p "Notes" text))
       ;; No "(none)" placeholders anywhere and no dim-face text left
       (should-not (string-match-p "(none)" text))
       (should-not (beads-show-test--find-text-with-face
...
(ert-deftest beads-show-test-render-metadata-absent-skipped ()
  "Test that an absent metadata map skips the METADATA section."
  (beads-show-test-with-temp-buffer
   (let ((parsed (beads--parse-issue beads-show-test--minimal-issue)))
     (beads-show--render-issue parsed)
     (should-not (string-match-p "METADATA"
...
(ert-deftest beads-show-test-render-lease-absent-skipped ()
  "Test that an issue without lease fields skips the LEASE section."
  (beads-show-test-with-temp-buffer
   (let ((parsed (beads--parse-issue beads-show-test--minimal-issue)))
     (beads-show--render-issue parsed)
     (should-not (string-match-p "LEASE"
                                 (beads-show-test--get-buffer-text))))))
...
(ert-deftest beads-show-test-render-tracks-empty-skipped ()
  "Test that absent tracking links skip both tracking sections."
  (beads-show-test-with-temp-buffer
   (let ((parsed (beads--parse-issue beads-show-test--minimal-issue)))
     (beads-show--render-issue parsed)
     (let ((text (beads-show-test--get-buffer-text)))
       (should-not (string-match-p "TRACKS" text))
       (should-not (string-match-p "TRACKED BY" text))
       (should-not (string-match-p "(none)" text))))))
...
(ert-deftest beads-show-test-render-comments-none-skipped ()
  "Test that an issue without comments skips the COMMENTS section."
  (beads-show-test-with-temp-buffer
   (let ((parsed (beads--parse-issue beads-show-test--minimal-issue)))
     (beads-show--render-issue parsed)
     (should-not (string-match-p "COMMENTS"
                                 (beads-show-test--get-buffer-text))))))
...
(ert-deftest beads-show-test-render-close-reason-empty-skipped ()
  "Test that a closed bead without a reason skips the CLOSE REASON section."
  (beads-show-test-with-temp-buffer
   (let ((parsed (beads--parse-issue beads-show-test--closed-issue)))
     (beads-show--render-issue parsed)
     (should-not (string-match-p "CLOSE REASON"
                                 (beads-show-test--get-buffer-text))))))
...
(ert-deftest beads-show-test-render-labels-absent-key-skipped ()
  "Test that an issue without a labels key skips the LABELS section.
The absent key and an explicitly empty label list get the same
hide-when-empty treatment: no header and no placeholder."
  (beads-show-test-with-temp-buffer
   ;; Fixture without any `labels' key at all
   (let ((parsed (beads--parse-issue beads-show-test--minimal-issue)))
     (beads-show--render-issue parsed)
     (should-not (string-match-p "LABELS"
                                 (beads-show-test--get-buffer-text))))))
