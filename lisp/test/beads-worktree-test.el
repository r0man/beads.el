...
(ert-deftest beads-worktree-test-auto-no-daemon-adds-flag ()
  "Test beads--build-command adds --no-daemon when auto option enabled."
  :tags '(:unit)
  (let ((beads-worktree-auto-no-daemon t)
        (beads-global-no-daemon nil)
...
(ert-deftest beads-worktree-test-auto-no-daemon-disabled ()
  "Test beads--build-command does not add --no-daemon when auto disabled."
  :tags '(:unit)
  (let ((beads-worktree-auto-no-daemon nil)
        (beads-global-no-daemon nil)
        (beads-global-no-auto-flush nil)
        (beads-global-no-auto-import nil)
        (beads-global-no-db nil)
...
(ert-deftest beads-worktree-test-auto-no-daemon-not-worktree ()
  "Test beads--build-command does not add --no-daemon when not in worktree."
  :tags '(:unit)
  (let ((beads-worktree-auto-no-daemon t)
        (beads-global-no-daemon nil)
        (beads-global-no-auto-flush nil)
...
(ert-deftest beads-worktree-test-global-no-daemon-takes-precedence ()
  "Test beads-global-no-daemon adds flag even when auto is disabled."
  :tags '(:unit)
  (let ((beads-worktree-auto-no-daemon nil)
        (beads-global-no-daemon t)  ; Explicitly set
        (beads-global-no-auto-flush nil)
        (beads-global-no-auto-import nil)
        (beads-global-no-db nil)
        (beads-global-sandbox nil)
...
(ert-deftest beads-worktree-test-info-normal-repo ()
  ...
  (let ((beads-global-no-daemon nil)
        (beads-worktree-auto-no-daemon nil)
        (message-output nil))
    (cl-letf (((symbol-function 'beads--in-git-worktree-p)
               (lambda () nil))
...
(ert-deftest beads-worktree-test-info-auto-no-daemon ()
  "Test beads-info shows auto-worktree --no-daemon."
  :tags '(:unit)
  (let ((beads-global-no-daemon nil)
        (beads-worktree-auto-no-daemon t)
        (message-output nil))
    (cl-letf (((symbol-function 'beads--in-git-worktree-p)
               (lambda () t))
              ((symbol-function 'beads--find-main-repo-from-worktree)
              ...
              ((symbol-function 'message)
               (lambda (fmt &rest args)
                 (setq message-output (apply #'format fmt args)))))
      (beads-info)
      (should (string-match-p "--no-daemon: enabled (auto-worktree)" message-output)))))

(ert-deftest beads-worktree-test-info-env-var ()
  "Test beads-info shows --no-daemon via environment variable."
  :tags '(:unit)
  (let ((beads-global-no-daemon nil)
        (beads-worktree-auto-no-daemon nil)
        (message-output nil))
    (cl-letf (((symbol-function 'beads--in-git-worktree-p)
              ...
              ((symbol-function 'message)
               (lambda (fmt &rest args)
                 (setq message-output (apply #'format fmt args)))))
      (beads-info)
      (should (string-match-p "--no-daemon: enabled (via BD_NO_DAEMON)" message-output)))))
