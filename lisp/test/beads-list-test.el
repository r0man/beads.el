...
(ert-deftest beads-list-test-workflow-to-close ()
  ...
  (with-temp-buffer
    ...
    ;; Verify d is bound to close (Pattern 1 context-aware action)
    (let ((binding (lookup-key beads-list-mode-map (kbd "d"))))
      (should (eq binding 'beads-actions-close)))))
...
;;; Pattern 2 Transient Suffix Tests

(ert-deftest beads-list-test-list-all-calls-refresh ()
  "Test that beads-list-all suffix calls beads-list--refresh."
  :tags '(:unit)
  (let ((refreshed nil)
        (displayed nil))
    (cl-letf (((symbol-function 'beads-list--refresh)
               (lambda (&optional spec)
                 (setq refreshed spec)))
              ((symbol-function 'beads-list--get-or-create-buffer)
               (lambda (_type &optional _dir) (current-buffer)))
              ((symbol-function 'beads-list--display-buffer)
              ...
              ((symbol-function 'transient-args)
               (lambda (_) '("--status=open" "--type=bug"))))
      (with-temp-buffer
        (beads-list-mode)
        (beads-list-all)))
    (should refreshed)
    (should (beads-issue-spec-p refreshed))
    (should (equal (oref refreshed status) "open"))
    (should (equal (oref refreshed type) "bug"))))

(ert-deftest beads-list-test-list-all-empty-args ()
  "Test that beads-list-all with no filters uses default spec."
  :tags '(:unit)
  (let ((refreshed nil))
    (cl-letf (((symbol-function 'beads-list--refresh)
               (lambda (&optional spec)
    ...
    (should refreshed)
    (should (beads-issue-spec-p refreshed))
    (should (null (oref refreshed status)))))

(ert-deftest beads-list-test-list-ready-calls-ready ()
  "Test that beads-list-ready suffix calls beads-issue-ready."
  :tags '(:unit)
  (let ((called nil)
        (displayed nil))
    (cl-letf (((symbol-function 'beads-issue-ready)
               (lambda (&optional _limit) (setq called t) nil))
              ((symbol-function 'beads-list--get-or-create-buffer)
               (lambda (_type &optional _dir) (current-buffer)))
              ((symbol-function 'beads-list--populate-buffer)
               (lambda (_issues _cmd &optional _obj) nil))
              ((symbol-function 'beads-list--display-buffer)
               (lambda (buf) (setq displayed buf)))
              ((symbol-function 'beads-check-executable)
               (lambda () t))
      ...
      (with-temp-buffer
        (beads-list-mode)
        (beads-list-ready-suffix)))
    (should called)))

(ert-deftest beads-list-test-list-blocked-calls-blocked ()
  "Test that beads-list-blocked suffix calls beads-blocked-issue-list."
  :tags '(:unit)
  (let ((called nil))
    (cl-letf (((symbol-function 'beads-blocked-issue-list)
