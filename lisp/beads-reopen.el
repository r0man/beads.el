...
;; Provides an interactive interface for reopening closed issues in Beads.
                ...
                (with-current-buffer buf
                  (cond
                   ...
                   ((and (derived-mode-p 'beads-show-mode)
                    ...
                    (beads-refresh-show)))))))
        (error
         (message "Failed to reopen issue: %s"
...
;;;###autoload (autoload 'beads-reopen "beads-reopen" nil t)
(transient-define-prefix beads-reopen (&optional issue-id)
  "Reopen a closed issue in Beads with an optional reason.

This transient menu provides an interactive interface for reopening
closed issues. The issue ID is detected from context (beads-list or
beads-show buffers) or prompted for completion.

Optional argument ISSUE-ID pre-fills the issue to reopen."
  :value (lambda () nil)
  (interactive
   (list (beads-reopen--detect-issue-id)))
